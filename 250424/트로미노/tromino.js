...
for(var x=0; x<n-1; x++){
    for(var y=0; y<m-1;y++){ //x,y기준
    var max = 0;

    // sum = grid[x][y] + grid[x][y+1];
    if(y-1>0) max = grid[x][y-1];
    if(x-1>0 && max<grid[x-1][y]) max = grid[x-1][y];
    if(x+1<m && max<grid[x+1][y]) max = grid[x+1][y];
    if(x-1>0&&y+1<n&&max<grid[x-1][y+1]) max = grid[x-1][y+1];
    if(x+1<m&&y+1<n&&max<grid[x+1][y+1]) max = grid[x+1][y+1];
    if(y+2<n&&max<grid[x+1]) max = grid[x][y+2];
